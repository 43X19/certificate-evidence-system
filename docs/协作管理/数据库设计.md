...
CREATE TABLE credential_roots (
  root_id BIGINT PRIMARY KEY AUTO_INCREMENT,
  batch_id BIGINT NOT NULL,
...
- `merkle_tree_nodes` 保存每个批次树的所有中间节点（`level`、`position_in_level`、`node_hash`、`parent_position`），用于按需推导任意证书的 Merkle Proof（路径上的兄弟节点哈希）。
- 验证某张证书时，验证方只需要该证书自身哈希 + 一份 Merkle Proof（长度约为 log2(N)，N 为该批次证书数），不需要拿到批次内其他证书的数据；重新计算得到的哈希需与 `credential_roots.merkle_root` 一致才算通过。
